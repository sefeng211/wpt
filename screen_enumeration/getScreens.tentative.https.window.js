...
promise_test(async testCase => {
  ...
  promise_rejects_dom(testCase, 'NotAllowedError', self.getScreens());
...
promise_test(async testCase => {
  ...
  assert_greater_than((await iwindow.getScreens()).length, 0);
  document.body.removeChild(iframe);
  assert_equals(await iwindow.getScreens(), undefined);
}, 'iframe window supports getScreens() while attached');
